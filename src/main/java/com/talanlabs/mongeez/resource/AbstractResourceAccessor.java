package com.talanlabs.mongeez.resource;
...
public abstract class AbstractResourceAccessor implements ResourceAccessor {
    ...
    protected String convertToPath(String relativeTo, String path) {
        ...
        if (baseUrl == null) {
            throw new MongeezException("Cannot find base path '" + relativeTo + "'");
        }
        String base;
        if (baseUrl.toExternalForm().startsWith("file:")) {
            File baseFile = new File(baseUrl.getPath());
            if (!baseFile.exists()) {
                throw new MongeezException("Base file '" + baseFile.getAbsolutePath() + "' does not exist");
            }
            if (baseFile.isFile()) {
